...
import Codec.CBOR.Encoding (encodeListLen, encodeMapLen, encodeString)
import Codec.CBOR.Write (toStrictByteString)
import Data.IPLD.CID (CID, Codec (DagCbor), newCidV1)
import Data.List.Split (chunksOf)
import Language.Marlowe.ICI.Cbor (makeCid)
import Language.Marlowe.ICI.PTree (PTree (..))
...
toCBOR chunkSize (Leaf vs) =
  let vss = chunksOf chunkSize $ reverse vs
      pack vs' next =
        let bytes =
              toStrictByteString
                . mconcat
...
toCBOR chunkSize (Node tree) =
  let children = M.map (toCBOR chunkSize) tree
      bytes =
        toStrictByteString $
